...
поиска известных уязвимостей в зависимостях, FND-07) в ТЕКУЩЕМ dev-venv на этой машине (не в
чистом throwaway venv — D-14). Команды, вывод и версии ниже взяты из реального прогона на этой
машине в рамках выполнения Phase 16 (2026-07-27) — состояние зависимостей зафиксировано на дату
этого нового прогона, а не унаследовано из Phase 11. Отдельная непрерывная проверка `pip-audit`
уже встроена в CI (`.github/workflows/ci.yml`, Phase 11 Plan 01) — эта страница документирует
именно текущее (на момент прогона) состояние зависимостей, а не заменяет CI-шаг (D-18).

## Процедура

### 1. Обновление pip-audit в dev-venv

```powershell
venv\Scripts\python.exe -m pip install --upgrade pip-audit
```

Результат: `pip-audit` уже был установлен в актуальной версии (`Requirement already satisfied`);
живая перепроверка версии — `2.10.1` (без изменений с Phase 11).
...
|----------|----------|----------|
| Инструмент | `pip-audit 2.10.1` | Официальный PyPA-инструмент, использует базу советов OSV/PyPI; версия подтверждена живым `pip show pip-audit` этой фазы (без изменений с Phase 11) |
...
| Дата разового прогона | 2026-07-27 | Выполнено в рамках Phase 16, Plan 02 (FND-07) |
...
- **Сноска про `pip` (Open Question 1, вне scope):** сам инструмент `pip` (версия `26.1.1` в этом
  venv — без изменений с Phase 11) имеет 2 известные уязвимости (`PYSEC-2026-196`), исправленные в
  `26.1.2` — это видно только при запуске `pip-audit` БЕЗ флага `-r` (full-environment режим).
  Живая перепроверка этой фазы подтвердила, что факты не изменились. `pip` как бинарник пакетного
...
- Непрерывная проверка (D-18) — отдельный шаг `pip-audit -r requirements.txt` уже встроен в
  `.github/workflows/ci.yml` (Phase 11 Plan 01) и запускается на каждый push/PR в `main`. Эта
  страница документирует только разовое ручное состояние на момент прогона, а не заменяет CI-шаг.
...
- `.github/workflows/ci.yml` — непрерывный CI-шаг `pip-audit` (D-09/D-18), не заменяет эту разовую
  проверку
- Предыдущий разовый прогон — 2026-07-16 (Phase 11, FND-03) — см. историю git этого файла
  (`git log -- "06 - Engine/Pip Audit Dependency Scan.md"`) для точного текста прошлой версии
  страницы; результат тогда тоже был чистым (0 уязвимостей)
